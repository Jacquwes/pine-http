...
namespace pine
{
  ...
  {
    if (is_closed.load() || read_pending.load())
      return;
  ...
  {
    if (is_closed.load() || write_pending.load() || raw_message.empty())
      return;
  ...
  {
    if (bool expected = false; is_closed.compare_exchange_strong(expected, true))
      return;
